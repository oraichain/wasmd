...
func TestCreateNonHeightConditionFilterTable(t *testing.T) {
	testCases := []struct {
	...
	}{
		{
			...
			expectedQuery: `filtered_tx_event_attributes as (select distinct tx_id FROM events JOIN filtered_heights fh ON (fh.rowid = events.tx_id) JOIN attributes ON (events.rowid = attributes.event_id) WHERE composite_key = $1 AND value::numeric >= $2 ) `,
			expectedVals:  []interface{}{"account.number", int64(2)},
		},
		{
			query:         "account.number >= 2 AND account.number < 5",
			expectedQuery: `filtered_tx_event_attributes as (select distinct tx_id FROM events JOIN filtered_heights fh ON (fh.rowid = events.tx_id) JOIN attributes ON (events.rowid = attributes.event_id) WHERE composite_key = $1 AND value::numeric >= $2 INTERSECT select distinct tx_id FROM events JOIN filtered_heights fh ON (fh.rowid = events.tx_id) JOIN attributes ON (events.rowid = attributes.event_id) WHERE composite_key = $3 AND value::numeric < $4 ) `,
		...
		},
		{
			...
			expectedQuery: `filtered_tx_event_attributes as (select distinct tx_id FROM events JOIN filtered_heights fh ON (fh.rowid = events.tx_id) JOIN attributes ON (events.rowid = attributes.event_id) WHERE composite_key = $1 AND value = $2 INTERSECT select distinct tx_id FROM events JOIN filtered_heights fh ON (fh.rowid = events.tx_id) JOIN attributes ON (events.rowid = attributes.event_id) WHERE composite_key = $3 AND value::numeric <= $4 INTERSECT select distinct tx_id FROM events JOIN filtered_heights fh ON (fh.rowid = events.tx_id) JOIN attributes ON (events.rowid = attributes.event_id) WHERE composite_key = $5 AND value = $6 ) `,
