...
if [[ $burn_vote_veto != $expected_burn_vote_veto ]]; then
  echo "gov params Upgrade Failed: burn_vote_veto mismatch" >&2; exit 1
fi

if [[ $expedited_min_deposit_amount -ne $expected_expedited_min_deposit_amount ]]; then
  echo "gov params Upgrade Failed: expedited_min_deposit_amount mismatch" >&2; exit 1
fi

if [[ $expedited_min_deposit_denom != $expected_expedited_min_deposit_denom ]]; then
  echo "gov params Upgrade Failed: expedited_min_deposit_denom mismatch" >&2; exit 1
fi
...
if [[ $min_deposit_amount != $expected_min_deposit_amount ]]; then
  echo "gov params Upgrade Failed: min_deposit_amount mismatch" >&2; exit 1
fi

if [[ $min_deposit_denom != $expected_min_deposit_denom ]]; then
  echo "gov params Upgrade Failed: min_deposit_denom mismatch" >&2; exit 1
fi

if [[ $min_deposit_ratio != $expected_min_deposit_ratio ]]; then
  echo "gov params Upgrade Failed: min_deposit_ratio mismatch" >&2; exit 1
fi

if [[ $min_initial_deposit_ratio != $expected_min_initial_deposit_ratio ]]; then
  echo "gov params Upgrade Failed: min_initial_deposit_ratio mismatch" >&2; exit 1
fi

if [[ $proposal_cancel_ratio != $expected_proposal_cancel_ratio ]]; then
  echo "gov params Upgrade Failed: proposal_cancel_ratio mismatch" >&2; exit 1
fi

if [[ $quorum != $expected_quorum ]]; then
  echo "gov params Upgrade Failed: quorum mismatch" >&2; exit 1
fi

if [[ $threshold != $expected_threshold ]]; then
  echo "gov params Upgrade Failed: threshold mismatch" >&2; exit 1
fi
