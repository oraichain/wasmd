...
Localrelayer is a local testing environment composed of two localoraichain instances connected by a relayer.

## Endpoints

| Chain ID         | Component  | Endpoint                 |
|------------------|------------|--------------------------|
| `localoraichain-a` | `RPC`      | <http://localhost:26657> |
| `localoraichain-a` | `REST/LCD` | <http://localhost:1317>  |
| `localoraichain-a` | `gRPC`     | <http://localhost:9090>  |
| `localoraichain-a` | `faucet`   | <http://localhost:8080>  |
| `localoraichain-b` | `RPC`      | <http://localhost:36657> |
| `localoraichain-b` | `REST/LCD` | <http://localhost:31317> |
| `localoraichain-b` | `gRPC`     | <http://localhost:39090> |
| `localoraichain-b` | `faucet`   | <http://localhost:38080> |
...
|------------------|---------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `localoraichain-a` | `validator-a` | *family album bird seek tilt color pill danger message abuse manual tent almost ridge boost blast high comic core quantum spoon coconut oyster remove*            |
| `localoraichain-a` | `faucet`      | *increase bread alpha rigid glide amused approve oblige print asset idea enact lawn proof unfold jeans rabbit audit return chuckle valve rather cactus great*     |
| `localoraichain-a` | `relayer`     | *black frequent sponsor nice claim rally hunt suit parent size stumble expire forest avocado mistake agree trend witness lounge shiver image smoke stool chicken* |
| `localoraichain-b` | `validator-b` | *family album bird seek tilt color pill danger message abuse manual tent almost ridge boost blast high comic core quantum spoon coconut oyster remove*            |
| `localoraichain-b` | `faucet`      | *increase bread alpha rigid glide amused approve oblige print asset idea enact lawn proof unfold jeans rabbit audit return chuckle valve rather cactus great*     |
| `localoraichain-b` | `relayer`     | *black frequent sponsor nice claim rally hunt suit parent size stumble expire forest avocado mistake agree trend witness lounge shiver image smoke stool chicken* |
...
```bash
 ⠿ Network localrelayer_localoraichain        Created
```

2. run the following containers:

```bash
 ⠿ Container localrelayer-localoraichain-b-1  Created
 ⠿ Container localrelayer-localoraichain-a-1  Created
...
d90ec29c7a6f   local:oraichain                  "/oraichain/setup.sh"      About a minute ago   Up 3 seconds   26656/tcp, 0.0.0.0:31317->1317/tcp, 0.0.0.0:39090->9090/tcp, 0.0.0.0:36657->26657/tcp   localrelayer-localoraichain-b-1
e36cead49a07   local:oraichain                  "/oraichain/setup.sh"      About a minute ago   Up 3 seconds   0.0.0.0:1317->1317/tcp, 0.0.0.0:9090->9090/tcp, 0.0.0.0:26657->26657/tcp, 26656/tcp     localrelayer-localoraichain-a-1
```
...
Check `localoraichain-a` status:

```bash
curl -s http://localhost:26657/status
```

Check `localoraichain-b` status:
...
```bash
❯ oraichaind keys add my-account --keyring-backend test
...
# Use the following endpoint for localoraichain-b:
...
hermes tx ft-transfer --timeout-seconds 1000 \
    --dst-chain localoraichain-a \
    --src-chain localoraichain-b \
    --src-port transfer \
