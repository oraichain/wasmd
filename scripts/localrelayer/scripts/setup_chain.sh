...
add_genesis_accounts() {
    ...
    oraid add-genesis-account $VALIDATOR_ACCOUNT 100000000000orai --home $ORAICHAIN_HOME
    ...
    oraid add-genesis-account $FAUCET_ACCOUNT 100000000000orai --home $ORAICHAIN_HOME
    ...
    oraid add-genesis-account $RELAYER_ACCOUNT 1000000000orai --home $ORAICHAIN_HOME

    oraid gentx $VALIDATOR_MONIKER 500000000orai --keyring-backend=test --chain-id=$CHAIN_ID --home $ORAICHAIN_HOME
    oraid collect-gentxs --home $ORAICHAIN_HOME
}
...
edit_config() {
    ...
    sed -i -e "s%^pruning *=.*%pruning = \"$pruning\"%; " $CONFIG_FOLDER/config.toml
    sed -i -e "s%^pruning-keep-recent *=.*%pruning-keep-recent = \"$pruning_keep_recent\"%; " $CONFIG_FOLDER/config.toml
    sed -i -e "s%^pruning-interval *=.*%pruning-interval = \"$pruning_interval\"%; " $CONFIG_FOLDER/config.toml
    # Expose the rpc
