...
if [[ $blocks_per_yer -ne 45051428 ]]; then
   echo "Mint params Upgrade Failed" >&2
   exit 1
fi

if awk "BEGIN { exit !($goal_bonded != 0.670000000000000000) }"; then
   echo "Mint params Upgrade Failed" >&2
   exit 1
fi

if awk "BEGIN { exit !($inflation_max != 0.070000000000000000) }"; then
   echo "Mint params Upgrade Failed" >&2
   exit 1
fi

if awk "BEGIN { exit !($inflation_min != 0.070000000000000000) }"; then
   echo "Mint params Upgrade Failed" >&2
   exit 1
fi

if awk "BEGIN { exit !($inflation_rate_change != 0.07) }"; then
   echo "Mint params Upgrade Failed" >&2
   exit 1
fi
