#!/usr/bin/env bash
# Snapshot ORAI + CW20 balances for localfork fork comparison.
...
ROWS='[]'
add_row() {
  local role="$1" addr="$2" orai="$3" cw20_a="$4" cw20_b="$5"
  ROWS=$(jq -nc --argjson rows "${ROWS}" \
    --arg role "${role}" --arg addr "${addr}" \
    --arg orai "${orai}" --arg cw20_a "${cw20_a}" --arg cw20_b "${cw20_b}" \
    '$rows + [{role:$role,address:$addr,orai:$orai,cw20_v1:$cw20_a,cw20_v2:$cw20_b}]')
}
...
jq -nc \
  ...
  '{phase:$phase,height:($height|tonumber),denom:$denom,cw20_contract_v1:$cw20_v1,cw20_contract_v2:$cw20_v2,cw20_mint_amount:$cw20_amount,wallets:$wallets}' \
  > "${OUT}"

echo "✓ wrote ${OUT} (height=${HEIGHT}, wallets=$(echo "${ROWS}" | jq 'length'))"
