...
alias chainA="oraid --node http://localhost:26654 --chain-id localoraichain-a"
alias chainB="oraid --node http://localhost:26651 --chain-id localoraichain-b"

# setup the keys
echo "bottom loan skill merry east cradle onion journey palm apology verb edit desert impose absurd oil bubble sweet glove shallow size build burst effort" | oraid --keyring-backend test keys add validator --recover || echo "key exists"
echo "increase bread alpha rigid glide amused approve oblige print asset idea enact lawn proof unfold jeans rabbit audit return chuckle valve rather cactus great" | oraid --keyring-backend test  keys add faucet --recover || echo "key exists"

VALIDATOR=$(oraid keys show validator -a)
