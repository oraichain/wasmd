...
VALIDATOR2_ARGS="--from validator2 --chain-id $CHAIN_ID -y --keyring-backend test --gas auto --gas-adjustment 1.5 --fees 10000orai -b sync --home $HOME/.oraid/validator2"
HIDE_LOGS="/dev/null"
...
sleep 2
proposal_id=$(oraid query tx $txhash --output json | jq -r '.events[] | select(.type == "proposal_deposit") | .attributes[] | select(.key == "proposal_id") | .value')
