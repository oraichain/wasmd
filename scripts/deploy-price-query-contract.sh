...
ARGS="--from validator1 --home $VALIDATOR_HOME --chain-id testing -y --keyring-backend test --fees 200orai --gas auto --gas-adjustment 1.5"
...
INSTANTIATE_MSG='{}'
instantiate_ret=$(oraid tx wasm instantiate $code_id "$INSTANTIATE_MSG" --label 'testing' --admin $(oraid keys show validator1 --keyring-backend test --home $VALIDATOR_HOME -a) $ARGS --output json)
...
oraid tx wasm execute $contract_address "$ADD_POOL" $ARGS --output json
