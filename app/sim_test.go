...
func TestAppImportExport(t *testing.T) {
	...
	newApp := NewWasmApp(log.NewNopLogger(), newDB, nil, true, appOptions, emptyWasmOpts, EvmAppOptions, fauxMerkleModeOpt, baseapp.SetChainID(SimAppChainID))
...
func setupSimulationApp(t *testing.T, msg string) (simtypes.Config, dbm.DB, simtestutil.AppOptionsMap, *WasmApp) {
	...
	app := NewWasmApp(logger, db, nil, true, appOptions, emptyWasmOpts, EvmAppOptions, fauxMerkleModeOpt, baseapp.SetChainID(SimAppChainID))
...
func TestAppStateDeterminism(t *testing.T) {
	...
	for i := 0; i < numSeeds; i++ {
		...
		for j := 0; j < numTimesToRunPerSeed; j++ {
			...
			db := dbm.NewMemDB()
			app := NewWasmApp(logger, db, nil, true, appOptions, emptyWasmOpts, EvmAppOptions, interBlockCacheOpt(), baseapp.SetChainID(SimAppChainID))
