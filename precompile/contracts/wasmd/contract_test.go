...
func TestUnmarshalCosmWasmDeposit(t *testing.T) {
	...
	coins := sdk.NewCoins(sdk.NewCoin("ukava", sdkmath.NewInt(10)), sdk.NewCoin("orai", sdkmath.NewInt(100))).Sort()
...
func TestExecute(t *testing.T) {
	...
	tApp := app.NewWasmAppWithCustomOptions(t, true, app.SetupOptions{
		Logger:  logger.With("instance", "first"),
		DB:      db,
		AppOpts: simtestutil.NewAppOptionsWithFlagHome(t.TempDir()),
	})
	ctx := tApp.NewContextLegacy(true, cmtproto.Header{Height: 1, ChainID: "kava-test", Time: time.Now().UTC()})
