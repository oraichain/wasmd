...
func (c Config) ValidateBasic() error {
	if err := c.EVM.Validate(); err != nil {
		return errorsmod.Wrapf(sdkerrors.ErrAppConfig, "invalid ethermint evm config value: %s", err.Error())
	}

	if err := c.JSONRPC.Validate(); err != nil {
		return errorsmod.Wrapf(sdkerrors.ErrAppConfig, "invalid ethermint json-rpc config value: %s", err.Error())
	}

	if err := c.TLS.Validate(); err != nil {
		return errorsmod.Wrapf(sdkerrors.ErrAppConfig, "invalid ethermint tls config value: %s", err.Error())
	}
