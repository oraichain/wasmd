...
// TestStartOrai is a basic test to assert that spinning up a Orai network with 1 validator works properly.
func TestInterchainAccountFromGaia(t *testing.T) {
	if testing.Short() {
		t.Skip()
	}
	...
	// Get ibc connection
	ibcConnection, err := r.GetConnections(ctx, eRep, gaia.Config().ChainID)
	require.NoError(t, err)

	// register ICA
	_, err = helpers.RegisterICA(t, ctx, gaia, gaiaUser.KeyName(), ibcConnection[0].ID)
	require.NoError(t, err)
	err = testutil.WaitForBlocks(ctx, 10, orai, gaia)

	require.NoError(t, err)
	icaAddress, err := helpers.QueryInterchainAccount(t, ctx, gaia, gaiaUser.FormattedAddress(), ibcConnection[0].ID)
	require.NoError(t, err)
	require.NotEmpty(t, icaAddress)
}
